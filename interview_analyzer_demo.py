...
def expression_score(mar: float, brow_dist_norm: float) -> float:
    """
    Returns 0..1 where higher means relaxed/confident expression.
    Heuristics:
      - MAR higher than smiling threshold -> positive
      - Brow_dist_norm small -> brow close (tension) -> reduce score
    """
    score = 0.6  # baseline
    if mar > 0.35:
        score += 0.35
...
def posture_score(pts: np.ndarray) -> float:
    """
    Compute posture proxy using forehead vs cheeks vertical separation normalized by face height.
    Smaller ratio (head lower relative to cheeks) -> slouched -> lower score.
    """
...
def hand_movement_score(mean_motion: float, frame_diag: float) -> float:
    """
    mean_motion: average per-sample-frame hand motion in pixels
    frame_diag: diagonal length of frame for normalization
    """
    if frame_diag <= 1e-6:
        return 1.0
